...
import subprocess
import sys
sys.path.append('E:/Programming/Python/MyOwnClass')
...
def typecheck(filelist):
    filetype = ['.mp4','.ml','.avi','.txt','.mp3','.aac']
...
"""             if  "audio:" in line:
                totalsize = re.search(r'audio:\d+',line)
                totalsize = re.findall(r'\d+',totalsize[0])
                progbar.setRange(0,int(totalsize[0]))
                print("total:" + str(int(totalsize[0])))
            else:
                pass

            if  "size=" in line and "N/A" not in line:
                presize = re.search(r'\d+',line)   #找到第一个数字，对应于帧数
                progbar.setValue(int(presize[0]))
                print(int(presize[0]))
            else:
                pass """
...
"""     进度条处理方法一
    for line in process.stdout:                       
            findframe = re.findall(r'frame=\d+',line)   #r指定搜索类型，raw, \S指没有空格，搜索后面不带空格的 \S, \d+多位数字
            if findframe:                               #当前处理帧数
                preframe = re.findall(r'\d+',findframe[0])
                progbar.setRange(0,allframes)
                progbar.setValue(int(preframe[0]))      #进度条设置
...
class VideoMerge(QMainWindow):      #新建的类是从QDialog继承下来的，将ui布局中的元素全部传递进来，作为MainWindow函数的参数
    ...
    #  def editchange(self):
    #     fn = self.ui.textEdit.toPlainText()
    #     if 0 == fn.find('file:///'):
    #         self.ui.textEdit.setText(fn.strip('file:///'))
    ...
    def getffmpeg(self):
        ...
        elif os.path.exists("E:/Programming/Python/ffmpeg/bin/ffmpeg.exe"):
            self.ffmpegpath = "E:/Programming/Python/ffmpeg/bin/ffmpeg.exe"
        else:
            QMessageBox.about(None,"Message","Can't find ffmpeg.exe")
    ...
    def textadd(self,nline):            #文本框中加入文件
        #self.ui.textBrowser.setText(str(self.ui.textBrowser.toPlainText()) + nline + "\n")  #此处是加上已经存在于框内的文字
    ...
    def find_file(self):
        ...
        #fname = QFileDialog.getOpenFileNames(None, "Please select a video file", self.filepath, "*.mp4;*.ml;*.txt")        
        ...
        if len(fname[0]) == 0 and self.ui.MyTBrowser.allfileslist == [] :   #文本框记录的列表为空
            ...
            for fn in fname[0]:
                ...
                if ".txt" in fn:
                    pass
                elif ".ml" in fn or ".mp4" in fn:           #若是视频文件，获取视频信息，并将txt文件地址变为ffmpeg可识别的内容格式
